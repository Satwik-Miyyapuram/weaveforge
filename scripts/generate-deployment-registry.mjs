...
const ROUTES_OUTPUT = join(ROOT, "apps/web/src/deployment/generated-routes.ts");
...
const routeImports = selectedRoutes.map(([, source, exportName, localName]) => `import { ${exportName} as ${localName} } from "${source}";`).join("\n");
const routeMap = selectedRoutes.map(([path,,,localName]) => `${JSON.stringify(path)}: ${localName}`).join(", ");
...
const routesOutput = `// AUTO-GENERATED by scripts/generate-deployment-registry.mjs — do not edit.\n${routeImports}\n\nexport const GENERATED_ROUTE_COMPONENTS = { ${routeMap} } as const;\n`;
...
for (const [routePath, relativeTarget] of Object.entries(generatedRoutePages)) {
  ...
  if (selectedRoutePaths.has(routePath)) {
    ...
    writeFileSync(target, `// AUTO-GENERATED by scripts/generate-deployment-registry.mjs — do not edit.\nimport { renderGeneratedRoute } from "@/deployment/render-route";\n\nexport default function GeneratedRoutePage() {\n  return renderGeneratedRoute(${JSON.stringify(routePath)});\n}\n`, "utf8");
...
writeFileSync(ROUTES_OUTPUT, routesOutput, "utf8");
