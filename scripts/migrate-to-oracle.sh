#!/usr/bin/env bash
# One command for the whole migration — the bash original.
#
#   set -a && source .env.migration && set +a
#   ./scripts/migrate-to-oracle.sh
#
# `npm run migrate` now runs migrate-to-oracle.mjs instead, which does the same
# thing, loads .env.migration itself, and works on Windows — where npm hands the
# script to cmd.exe and `./x.sh` is a syntax error. This is kept for anyone who
# prefers it; the two are expected to stay in step.
#
...
if ! node scripts/migration-preflight.mjs; then
  ...
  if [[ "${DATABASE_URL:-}" != "" ]] && \
     ! node -e "
       ...
       const {Client}=createRequire('./apps/web/package.json')('pg');
    ...
    node scripts/apply-migrations-oci.mjs || die "Schema failed to apply."
