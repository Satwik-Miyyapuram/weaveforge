...
7. **Deploy** — set `NEXT_PUBLIC_BACKEND_PROVIDER=postgres` and `DATABASE_URL` for **server-side** code. The browser needs a data API of its own; see [`oracle-shift-guide.md`](oracle-shift.md).
...
- [`plans/completed/migration-plan.md`](../internal/plans/completed/migration-plan.md) — phased self-host plan
- [DESIGN.md](../building/design.md) — SOLID, composition root, repository contracts
- [integrations.md](../using/integrations.md) — third-party services (Zotero, GitLab, …)
- [dev.md](../building/dev.md) — feature modules and facades
