...
- **`docs/internal/strategy/competitive-scan.md`** + **`docs/internal/plans/completed/competitive-scan-implementation-plan.md`**: product steal list + phased build plan.

## Adding a New Component or Feature

**Start here:** [`docs/using/extensions.md`](../using/extensions.md) for the full touch-point checklist. Below is a minimal timer example.
...
5. **Wire domain logic**
   Add use-cases in `packages/core`, wire in `bootstrap.ts`, expose via a facade in `container/facades/`. See [`extensions.md`](../using/extensions.md) §4.
...
**Full guide:** [`docs/using/integrations.md`](../using/integrations.md)
...
- **A file that outgrows itself becomes a folder**, named for the pieces it
  ...
  examples; see [`docs/building/design.md`](design.md) § 3.2.2.
...
| Gate | What it runs |
| --- | --- |
| `npm run check:solid` | `node scripts/check-solid.mjs` |
...
**Releases:** Git tags / PyPI = Python SDK only. See [`docs/building/release.md`](release.md).
...
Use the components in `apps/web/src/components/` (like `<Select>`, `<Modal>`, etc) to ensure the UI remains visually consistent with the `docs/building/themes.md` design system. Avoid writing ad-hoc inline styles unless dealing with absolute edge-case positioning.
