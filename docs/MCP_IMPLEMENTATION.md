...
Read-only tools operate only on granted sources: paper metadata and notes,
Zotero annotations and notes, reading lists, vault notes, logbook entries,
experiments and milestones. PDFs, report content, settings, credentials and
unselected sources are excluded.
...
A typed browser-local executor performs the normal app write after approval,
and an encrypted audit entry records the outcome.
