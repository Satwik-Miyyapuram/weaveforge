...
[`../../pricing-strategy.md`](../../strategy/pricing.md) §3.1.
...
- **The opt-in is one flow with four steps, in this order**, and it is the only
  ...
  2. **count what would be uploaded and check it against the limit** — papers,
     ...
     sent (`docs/internal/future-work/billing-and-quota-plan.md` §3.1: the check happens
...
(`docs/internal/strategy/pricing.md` §1: we sell the operator's job, not a capability).
...
decorator in its composition root (`docs/internal/future-work/billing-and-quota-plan.md`
...
- **Sign-out must offer a choice**, and must not assume: keep the local data
  ...
  `docs/internal/reports/privacy-test-matrix.md`.
