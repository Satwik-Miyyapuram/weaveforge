 ...
 * order, from the candidates this build can offer. Every candidate reads on this
 * machine; there is no longer one that can be switched on to send strokes away.
 */
...
export class InkFacade {
  ...
  async candidates(): Promise<InkRecogniser[]> {
    return inkRecogniserCandidates({ bridge: this.deps.bridge });
  }
