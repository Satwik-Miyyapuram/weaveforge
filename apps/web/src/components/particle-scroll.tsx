...
/**
 * Particle scroll — vendored from Canvas UI (`@canvas-ui/particle-scroll-react`,
 * https://canvasui.dev/docs/components/particle-scroll), not installed.
 *
 * The published route is `npx shadcn@latest add`, which needs a `components.json`
 * and a Tailwind pipeline. This app has neither: styling is `globals.css` plus
 * CSS modules, and `check:ui` enforces that. The component itself turned out to
 * need no part of that toolchain — no dependencies, no Tailwind classes, one
 * `className` passthrough — so the file is copied in whole and reviewed rather
 * than dragging a second styling system into the tree for one effect.
 *
 * Local change, marked `WEAVEFORGE:` below: upstream dissolves only what sits
 * *below* the formation line, so material scrolling off the top stayed sharp.
 * `focus: "both"` fades on either side of the line, which is what "only the
 * thing being read is solid" actually requires.
 *
 * Rendering needs WebGL2 *and* the HTML-in-Canvas API (`drawElementImage` /
 * `requestPaint`), which in August 2026 is a Chromium origin trial behind
 * `chrome://flags/#canvas-draw-element`. Firefox and Safari have not shipped it.
 * Unsupported browsers render the children directly — no particles, nothing
 * broken — and `prefers-reduced-motion` assembles instantly.
 */
...
export interface ParticleScrollOptions {
  ...
  smoothing?: number;
  /**
   * WEAVEFORGE: which side of the formation line dissolves.
   *
   * `"below"` is upstream behaviour — content assembles once it rises past the
   * line and stays solid from then on. `"both"` also dissolves it again as it
   * leaves through the top, so only what is level with the line is ever solid.
   */
  focus?: "below" | "both";
  /**
   * WEAVEFORGE: height in CSS pixels, centred on the formation line, that stays
...
const DEFAULTS: Required<ParticleScrollOptions> = {
  ...
  settle: 1.2,
  smoothing: 0.6,
  // WEAVEFORGE: defaults keep upstream behaviour for anyone using the component
  // as published; the pitch page opts into "both".
  focus: "below",
  keep: 0,
};
...
export function createParticleScroll(
  ...
  function syncBgColor() {
    ...
    while (el) {
      ...
      if (css && css !== "transparent") {
        ...
        const pixel = bgCtx.getImageData(0, 0, 1, 1).data;
        const r = pixel[0] ?? 0;
        const g = pixel[1] ?? 0;
        const b = pixel[2] ?? 0;
        const a = pixel[3] ?? 0;
  ...
  function rowTargetFor(docRowY: number) {
    ...
    // WEAVEFORGE: symmetric focus. Upstream measures distance from the line in
    // one direction only, so a row that had assembled stayed assembled forever
    // once it passed. Measuring the absolute distance instead lets a row fade
    // back out through the top exactly as it faded in from the bottom, leaving
    // the band around the line as the only solid region on screen.
    if (config.focus === "both") {
      const keep = Math.max(config.keep ?? 0, 0) / 2;
      const distance = Math.abs(vy - line) - keep;
      return Math.min(Math.max(1 - distance / band, 0), 1);
    }
