...
test("bounds arithmetic is inclusive and slack grows the box", () => {
  const bounds = boundsOf(
    Float32Array.from([10, 40, 25]),
    Float32Array.from([5, 60, 30]),
  );
  assert.deepEqual(bounds, [10, 5, 40, 60]);
  assert.equal(
    boundsIntersect(bounds, [40, 60, 90, 90]),
    true,
    "touching counts as intersecting",
  );
  assert.equal(boundsIntersect(bounds, [41, 61, 90, 90]), false);
  assert.equal(
    boundsIntersect(bounds, [45, 65, 90, 90], 5),
    true,
    "slack reaches it",
  );
  assert.equal(
    boundsContain(bounds, 10, 5),
    true,
    "the box contains its own corner",
  );
...
test("a page round-trips through the chunk with its geometry intact", () => {
  ...
  assert.equal(
    again.stroke(1)!.width,
    60,
    "a 6 mm highlighter keeps its width through the buffer",
  );
...
test("an erase is a bit, not a splice: indices stay stable", () => {
  ...
  assert.equal(
    buffer.erase(1),
    false,
    "erasing twice is a no-op, not an error",
  );
...
test("compaction renumbers, and says how", () => {
  ...
  assert.deepEqual(
    [...remap.entries()],
    [[0, 1]],
    "and the map says where it came from",
  );
});
...
test("the page's bounds cover every live stroke and ignore dead ones", () => {
  const buffer = new InkPageBuffer(
    pageOf(line(100, 300, 4), line(900, 1500, 4)),
  );
  ...
  assert.deepEqual(
    buffer.bounds(),
    [100, 300, 160, 300],
    "an erased stroke does not extend the page",
  );
});
...
test("a stroke with no points is held without breaking the bounds", () => {
  ...
  assert.deepEqual(
    buffer.bounds(),
    [0, 0, 0, 0],
    "an empty stroke does not move the page's box",
  );
});
...
test("pressure that arrived as nonsense is stored as none", () => {
  ...
  assert.deepEqual(
    Array.from(geometry.pressure),
    [0, 0, 255],
    "clamped, with NaN read as no sample",
  );
});
...
test("a chunk written from a buffer is a chunk the container reads back", () => {
  ...
  const chunk = new Uint8Array(
    INK_CHUNK_HEADER_BYTES + encodeInkChunkBody(buffer.toPage()).length,
  );
  ...
  assert.deepEqual(
    Array.from(back.stroke(0)!.x),
    Array.from(buffer.stroke(0)!.x),
  );
