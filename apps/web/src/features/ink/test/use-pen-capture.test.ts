...
import { InkSamplePool, InkSampleWriter, type InkWorkerMessage } from "../application/capture-protocol";

/** A pointer event with sane defaults, so each test states only what it means. */
function pointer(overrides: Partial<PenPointerEvent> & { pointerType: string }): PenPointerEvent {
  return {
    pointerId: 1,
    pressure: 0.5,
    width: 1,
...
test("contact area is converted from the CSS pixels a pointer reports", () => {
  assert.equal(contactAreaMm2({ width: 1, height: 1 }), (1 / CSS_PX_PER_MM) ** 2);
  assert.equal(contactAreaMm2({ width: 0, height: 10 }), 0, "a device with no contact box has none");
...
test("a pen draws immediately, and after it a touch only scrolls", () => {
  ...
  gate.reset();
  assert.equal(gate.decide(pointer({ pointerType: "mouse" })), "draw", "a mouse is not a palm");
});
...
test("a second touch while one is drawing cancels a young first stroke", () => {
  const gate = new InkPenGate();
  const first = gate.begin(pointer({ pointerType: "touch", pointerId: 1, t: 0 }));
  assert.equal(first.decision, "defer");
  // A second contact 100 ms later: neither was writing.
  const second = gate.begin(pointer({ pointerType: "touch", pointerId: 2, t: 100 }));
  assert.equal(second.decision, "ignore");
  assert.deepEqual(second.cancelled, [1], "the young first stroke is thrown away");
  assert.equal(gate.activePointerId, null);
});
...
test("a second touch after a stroke is established leaves it alone", () => {
  ...
  assert.equal(gate.activePointerId, 1);
  const second = gate.begin(pointer({ pointerType: "pen", pointerId: 2, t: PALM_SECOND_TOUCH_MS + 10 }));
  assert.equal(second.decision, "ignore", "one stroke at a time");
  assert.deepEqual(second.cancelled, []);
  assert.equal(gate.activePointerId, 1, "and the stroke in progress keeps the pointer");
});
...
test("a touch in the writing hand's quadrant near a pen down is a palm", () => {
  ...
  assert.equal(
    gate.palmReason(pointer({ pointerType: "touch", clientX: 800, clientY: 700, bounds, t: 1_100 })),
    "quadrant",
  );
  // The other side of the page is not where a right hand rests.
  assert.equal(
    gate.palmReason(pointer({ pointerType: "touch", clientX: 200, clientY: 200, bounds, t: 1_100 })),
    null,
  );
  // A pen only classifies a *touch*: a stylus is never a palm.
  assert.equal(gate.palmReason(pointer({ pointerType: "pen", clientX: 800, clientY: 700, bounds })), null);
});
...
test("a left-handed writer's hand is on the other side", () => {
  ...
  gate.notePenSeen(1_000);
  assert.equal(
    gate.palmReason(pointer({ pointerType: "touch", clientX: 100, clientY: 700, bounds, t: 1_100 })),
    "quadrant",
  );
  assert.equal(
    gate.palmReason(pointer({ pointerType: "touch", clientX: 900, clientY: 700, bounds, t: 1_100 })),
    null,
  );
});
...
test("the quadrant rule lapses outside the pen's window, and without a box", () => {
  ...
  gate.notePenSeen(1_000);
  assert.equal(
    gate.palmReason(pointer({ pointerType: "touch", clientX: 800, clientY: 700, bounds, t: 5_000 })),
    null,
    "ten seconds after the last pen down, a hand has moved on",
  );
  assert.equal(
    gate.palmReason(pointer({ pointerType: "touch", clientX: 800, clientY: 700, t: 1_100 })),
    null,
    "with no surface box the rule is skipped rather than guessed at",
  );
  const never = new InkPenGate({ handedness: "right" });
  assert.equal(
    never.palmReason(pointer({ pointerType: "touch", clientX: 800, clientY: 700, bounds, t: 1_100 })),
    null,
...
test("a pen takes the surface from a touch, and the touch's stroke is discarded", () => {
  ...
  const palm = gate.begin(pointer({ pointerType: "touch", pointerId: 21, clientX: 800, clientY: 700, bounds, t: 0 }));
  assert.equal(palm.decision, "defer", "a touch with no pen yet is given its 120 ms");
  // The pen arrives 80 ms later.
  const pen = gate.begin(pointer({ pointerType: "pen", pointerId: 22, clientX: 100, clientY: 100, bounds, t: 80 }));
...
test("the wrist guard overrides every touch rule", () => {
  ...
  assert.equal(gate.decide(pointer({ pointerType: "pen" })), "draw");
  gate.penOnly = false;
  assert.equal(gate.decide(pointer({ pointerType: "touch" })), "ignore", "the pen has still been seen");
...
function sessionHarness(options: { delegating?: boolean; project?: (x: number, y: number) => { x: number; y: number } | null } = {}): SessionHarness {
  ...
  harness.session.setTool({ width: 6, tool: "pen", colour: "text", pageIndex: 0 });
  return harness;
}
...
test("a 240 Hz stroke through the session touches the screen once, at the end", () => {
  ...
  for (let i = 1; i <= 120; i += 1) {
    ...
    harness.session.pointerRawUpdate(
      pointer({ pointerType: "pen", pointerId: 7, clientX: 100 + i, clientY: 100 + i, t: at }),
    );
    harness.session.flush();
  }
  assert.equal(harness.committed.length, 0, "nothing has asked the screen to render yet");
  assert.ok(harness.trail > 100, "the trail followed every sample, in a ref");

  const up = pointer({ pointerType: "pen", pointerId: 7, clientX: 220, clientY: 220, t: 484 });
  harness.session.pointerUp(up);
  assert.equal(harness.session.active, false);
  assert.equal(harness.committed.length, 0, "and still nothing until the worker answers");
...
test("coalesced events are consumed with the dispatched one, not instead of it", () => {
  const harness = sessionHarness();
  harness.session.pointerDown(pointer({ pointerType: "pen", pointerId: 3, t: 0 }));
  const dispatched = pointer({
    ...
    getCoalescedEvents: () => [
      ...
      pointer({ pointerType: "pen", pointerId: 3, clientX: 250, clientY: 250, t: 14 }),
    ],
  });
  harness.session.pointerRawUpdate(dispatched);
  harness.session.pointerUp(pointer({ pointerType: "pen", pointerId: 3, clientX: 300, clientY: 300, t: 24 }));
  ...
  const counts = harness.messages
    .filter((message) => message.type === "stroke-begin" || message.type === "samples" || message.type === "stroke-end")
    .reduce((sum, message) => sum + ("sample" in message ? message.sample.count : 0), 0);
...
test("predicted samples ride the live tail and are never part of the stroke", () => {
  ...
  harness.session.pointerDown(pointer({ pointerType: "pen", pointerId: 4, t: 0 }));
  harness.session.pointerRawUpdate(
    pointer({
      pointerType: "pen",
      pointerId: 4,
      ...
      clientY: 120,
      t: 8,
      getPredictedEvents: () => [pointer({ pointerType: "pen", pointerId: 4, clientX: 160, clientY: 160, t: 16 })],
    }),
  );
...
test("while delegating, prediction is not used at all", () => {
  ...
  harness.session.pointerDown(pointer({ pointerType: "pen", pointerId: 5, t: 0 }));
  harness.session.pointerRawUpdate(
    pointer({
      pointerType: "pen",
      pointerId: 5,
      ...
      clientY: 120,
      t: 8,
      getPredictedEvents: () => [pointer({ pointerType: "pen", pointerId: 5, clientX: 160, clientY: 160, t: 16 })],
    }),
  );
  const predicted = harness.messages.filter(
    (message) => message.type === "samples" && message.predicted,
  );
  assert.equal(predicted.length, 0, "the OS is drawing ahead; predicting too would fork the stroke");
});

test("a late palm decision discards the stroke in progress", () => {
  const harness = sessionHarness();
  harness.session.pointerDown(pointer({ pointerType: "touch", pointerId: 9, t: 0 }));
...
test("a second contact cancels a young touch stroke", () => {
  const harness = sessionHarness();
  harness.session.pointerDown(pointer({ pointerType: "touch", pointerId: 11, t: 0 }));
  harness.session.confirmTouch({ x: 200, y: 200 }, pointer({ pointerType: "touch", pointerId: 11, t: 4 }));
  assert.equal(harness.session.active, true, "the touch earned its stroke by moving");

  harness.session.pointerDown(pointer({ pointerType: "touch", pointerId: 12, t: 100 }));
  assert.deepEqual(harness.cancelled, [11], "the screen was told to discard it");
  assert.equal(harness.session.active, false);
});

test("a stroke that never leaves the page is never sent anywhere", () => {
  const harness = sessionHarness({ project: () => null });
  harness.session.pointerDown(pointer({ pointerType: "pen", pointerId: 13, t: 0 }));
  harness.session.pointerRawUpdate(pointer({ pointerType: "pen", pointerId: 13, t: 8 }));
  harness.session.flush();
  assert.equal(harness.messages.length, 0, "an off-page sample projects to nothing");
  harness.session.pointerUp(pointer({ pointerType: "pen", pointerId: 13, t: 16 }));
});
