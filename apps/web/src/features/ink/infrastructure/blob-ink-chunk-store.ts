...
export class BlobInkChunkStore implements InkChunkStore {
  ...
  async read(noteId: string, chunkId: string): Promise<Uint8Array | null> {
    try {
      return await this.blobs.fetchBytes(this.bucket, await this.path(noteId, chunkId));
    } catch {
  ...
  async write(noteId: string, chunkId: string, bytes: Uint8Array): Promise<void> {
    const blob = new Blob([bytes as BlobPart], { type: INK_CHUNK_CONTENT_TYPE });
    await this.blobs.upload(this.bucket, await this.path(noteId, chunkId), blob, INK_CHUNK_CONTENT_TYPE);
  }
