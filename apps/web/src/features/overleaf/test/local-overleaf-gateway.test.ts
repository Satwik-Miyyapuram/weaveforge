...
/** The first statement run, asserted to exist rather than indexed blindly. */
function first(calls: { sql: string; params: unknown[] }[]) {
  const call = calls[0];
  assert.ok(call, "expected a statement to have been run");
  return call;
}
