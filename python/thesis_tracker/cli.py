"""``thesis-tracker`` command-line interface.
...
``connect`` / ``track`` the library exposes — no business logic of its own.
    ...
    thesis-tracker list [--project NAME]
    thesis-tracker import-tb  <logdir>          [--name N] [--project P]
    thesis-tracker import-wandb <entity/proj/id> [--name N] [--project P]
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="thesis-tracker", description=__doc__)
